...
def load_vgg16_model(include_top):
    model = VGG16(weights='imagenet', include_top=include_top, input_shape=(224, 224, 3))
    return model


def build_model(number_of_classes, task, descriptor_size, activation_function, convolution_groups):
    ...
    if task in ['2b', '2c']:
        # override parameters for these task
        descriptor_size = 4096
        activation_function = 'relu'
        convolution_groups = 5
    ...
    # set base model layers as nontrainable
...
def fit_model(model, task, train_seq, val_seq, output_dir, loss_function):

    if task in ['2b', '2c']:
        loss_function = 'categorical_crossentropy'

    early_stopping = EarlyStopping(patience=1, min_delta=0.005)
...
def main():
    ...
    model = build_model(number_of_classes, args.task, args.descriptor_size, args.activation_function, args.convolution_groups)
    ...
    if args.task == '2a':
        ...
        model = fit_model(model, args.task, train_seq, val_seq, args.output_dir, args.loss_function)
